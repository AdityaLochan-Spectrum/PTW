...
@Service
public class WFModifytoSummary {

    private static final String WF_ID = "W000171";
    private static final String WF_DESC = "RFI";

    private static final String PROC_ID = "P000740";
    private static final String PROC_DESC = "Summary";
    ...
    @Transactional
    public void insertModifyToSummaryRow(Long rfiId) {
        BigDecimal sr = jdbc.queryForObject(
                "SELECT COALESCE(MAX(wpt_tracking_sr_no), 0) + 1 FROM [qbo].WorkflowProgressionTracking",
                BigDecimal.class);

        LocalDateTime now = LocalDateTime.now();
        LocalDateTime edc = now.plusDays(2); // or use exact EDC from your earlier sample if needed

        insertTrackingRow(sr, PROC_ID, PROC_DESC, PRIME_EMP, rfiId, now, edc, null);
    }
